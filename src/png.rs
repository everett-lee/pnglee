...
impl Png {
    ...
    fn chunk_type_to_first_index(&self, chunk_type: &str) -> Option<usize> {
        self.chunks
            .iter()
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        match self.chunk_type_to_first_index(chunk_type) {
            ...
            None => return Err(anyhow!("No matching chunk for {:?}", chunk_type)),
        }
    }
...
mod tests {
    ...
    fn test_from_combined_chunks() {
        ...
        let combined: Vec<u8> = chunk_data_one
            .iter()
            .chain(chunk_data_one.iter())
            .cloned()
            .collect();
        let with_header: Vec<u8> = Png::STANDARD_HEADER
            .iter()
            .chain(combined.iter())
            .cloned()
            .collect();
